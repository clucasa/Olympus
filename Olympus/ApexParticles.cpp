	...
	physx::apex::NxUserRenderer* renderer, NxModuleIofx* iofxModule)
{
    ...
    if(descParams->areParamsOK())
    {
        ...
        if(emitterActor)
        {
            emitterActor->setCurrentPosition(PxVec3(10.0f, 1.0f, 5.0f));
...
{
	HRESULT hr = D3DX11CreateShaderResourceViewFromFile(mDev, "Media/Textures/SoftParticle.dds"/*popcorn.png"*/, 0, 0, &spriteTexture, 0 );
